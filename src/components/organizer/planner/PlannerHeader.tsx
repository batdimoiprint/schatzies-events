        ...
        <div>
          ...
          <p className="text-[11px] text-white/80 flex items-center gap-1.5 flex-wrap">
            ...
            {(selectedProject.eventTime ||
              ...
              (selectedProject.rawStartDate && selectedProject.rawStartDate.includes('T'))) && (
              <>
                ...
                <span className="font-medium text-white">
                  {selectedProject.eventTime
                    ...
                    : selectedProject.rawStartDate?.includes('T')
                      ? formatTo12Hour(selectedProject.rawStartDate.split('T')[1].substring(0, 5))
                      : ''}
                  {(selectedProject.eventTime && selectedProject.eventEndTime) ||
                  (selectedProject.rawEndDate && selectedProject.rawEndDate.includes('T'))
                    ? ' - '
                    : ''}
                  {selectedProject.eventEndTime
                    ? formatTo12Hour(selectedProject.eventEndTime)
                    : selectedProject.rawEndDate?.includes('T')
                      ? formatTo12Hour(selectedProject.rawEndDate.split('T')[1].substring(0, 5))
                      : ''}
                </span>
              </>
            )}

    ...
    <section className="rounded-2xl border border-[#ddd8e8] bg-[#f6f4f7] p-4 shadow-[0_6px_14px_rgba(31,18,54,0.05)]">
      <div className="mb-8 mx-auto w-full max-w-2xl">
        {!isInlineNoteOpen ? (
          ...
          <div className="flex flex-col gap-2 rounded-xl border border-[#e3ddea] bg-white p-4 shadow-lg">
            {noteDraftError && <p className="px-1 mb-2 text-xs font-bold text-[#d22067]">{noteDraftError}</p>}
      ...
      <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4 space-y-4">
        {plannerNotes.map((note, index) => {
          ...
          return (
            <article key={note.id} draggable onDragStart={(e) => handleNoteDragStart(e, note.id)} onDragEnd={handleNoteDragEnd} onDragOver={(e) => { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; }} onDrop={(e) => handleNoteDrop(e, note.id)} onClick={() => handleEditPlannerNote(note)} className={`break-inside-avoid relative flex cursor-pointer flex-col overflow-hidden rounded-xl border border-[#e3ddea] bg-white transition-all hover:shadow-md ${noteTheme.shellClassName} ${isDragging ? 'opacity-30 border-dashed scale-95' : 'opacity-100'}`}>
              {note.imageDataUrl && <img src={note.imageDataUrl} alt={`${note.title} attachment`} className="w-full h-auto object-contain border-b border-[#e3ddea]" />}
              <div className="p-4 flex flex-col gap-2">
                {note.title && <p className="text-[15px] font-bold text-inherit">{note.title}</p>}
                <p className={`break-words whitespace-pre-wrap text-[13px] leading-relaxed text-inherit/90`}>{note.body}</p>
              </div>
            </article>
          );
        })}

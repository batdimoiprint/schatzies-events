...
export function VendorsTab({
  ...
  const typeColorMap = useMemo(() => {
    ...
    allServiceTypes.forEach((t, i) => {
      m[t.toLowerCase()] = getTypeColor(i);
    });
    return m;
  }, [allServiceTypes]);
  ...
  const assignedByType = useMemo(() => {
    ...
    for (const v of eventVendors) {
      const normalized = allServiceTypes.find(
        (t) => t.toLowerCase() === (v.service || '').toLowerCase()
      );
            ...
            <Card key={type} className="border-[#e7dfef] bg-white py-0 shadow-sm overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  <span
                    className="inline-flex size-8 items-center justify-center rounded-lg text-white text-[11px] font-black"
                ...
                {isFilled && (
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-[#e6f4e8] px-3 py-1 text-[10px] font-bold text-[#2e6b37] uppercase">
                    <span className="size-1.5 rounded-full bg-[#2ec24f]" />
                    Active
                  </span>
                )}
              </div>

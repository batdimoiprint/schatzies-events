...
import { ArrowUpDown, ArrowUp, ArrowDown, CalendarDays, X } from 'lucide-react';
...
export function EventManagerPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    const matchesSearch = (fields: string[]) =>
      ...
      fields.some((f) => f.toLowerCase().includes(normalizedSearchTerm));
  ...
  }, [activeTab, events, vendors, workers, searchTerm, sortKey, sortDirection]);
              ...
              {filteredData.activeTab === 'Events'
                ? filteredData.data.map((event) => (
                    ...
                    >
                      ...
                      <TableCell className="py-4 font-semibold text-[#5c546a] hidden md:table-cell">{event.venue}</TableCell>
